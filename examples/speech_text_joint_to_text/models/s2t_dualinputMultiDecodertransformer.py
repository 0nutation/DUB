...
class TransformerMultiInputMultiDecoder(FairseqDecoder):
    def __init__(
        self,
        dictionary,
        spch_decoder,
        text_decoder,
        spch_specific_decoder,
        text_specific_decoder,
        ...
        self.spch_specific_decoder = spch_specific_decoder
        self.text_specific_decoder = text_specific_decoder
        ...
        #gate mechanism
        assert spch_decoder.output_embed_dim == text_decoder.output_embed_dim
        assert spch_decoder.output_embed_dim == spch_specific_decoder.output_embed_dim
        assert spch_specific_decoder.output_embed_dim == text_specific_decoder.output_embed_dim


        self.w_gate = nn.Linear(in_features=spch_decoder.output_embed_dim*2, out_features=spch_decoder.output_embed_dim, bias=True)
        self.final_proj = nn.Linear(in_features=spch_decoder.output_embed_dim, out_features=len(dictionary), bias=False)
        self.final_proj.weight = spch_decoder.embed_tokens.weight
        ...
        **kwargs
        ...
        if isinstance(encoder_out, tuple):  # training with mulitple input
            ...
            for i, eo in enumerate(encoder_out):
                ...
                if i == 0:
                    speech_shared_rst = self.spch_decoder(prev_output_tokens, eo, incremental_state ,True)[0]
                    speech_specific_rst = self.spch_specific_decoder(prev_output_tokens, eo, incremental_state ,True)[0]
                    assert speech_shared_rst.shape[-1]==self.spch_decoder.output_embed_dim
                    gate = torch.sigmoid(self.w_gate(torch.cat((speech_shared_rst,speech_specific_rst),dim=-1)))
                    speech_output = self.final_proj((1-gate)*speech_shared_rst + gate*speech_specific_rst)
                    rst.append(
                        [speech_output]
                    )
                else:
                    text_shared_rst = self.text_decoder(prev_output_tokens, eo, incremental_state ,True)[0]
                    text_specific_rst = self.text_specific_decoder(prev_output_tokens, eo, incremental_state ,True)[0]
                    assert text_shared_rst.shape[-1]==self.text_decoder.output_embed_dim
                    gate = torch.sigmoid(self.w_gate(torch.cat((text_shared_rst,text_specific_rst),dim=-1)))
                    text_output = self.final_proj((1-gate)*text_shared_rst + gate*text_specific_rst)
                    rst.append(
                        [text_output]
                    )
...
@register_model("dual_input_multi_decoder_s2t_transformer")
class DualInputMultiDecoderS2TTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_decoder(cls, args, task):
        ...
        text_specific_decoder = TransformerDecoder(dec_cfg, task.target_dictionary, dec_emb)
        spch_specific_decoder = TransformerDecoder(dec_cfg, task.target_dictionary, dec_emb)

        spch_decoder = TransformerMultiInputMultiDecoder.share_spchdecoder(
            args, text_decoder, spch_decoder
        )
        decoder = TransformerMultiInputMultiDecoder(
            dictionary=task.target_dictionary,
            spch_decoder=spch_decoder,
            text_decoder=text_decoder,
            spch_specific_decoder=spch_specific_decoder,
            text_specific_decoder=text_specific_decoder,
    ...
    def build_model(cls, args, task):
        ...
        dualinputmultidecoders2ttransformer_base(args)
...
@register_model_architecture(
    "dual_input_multi_decoder_s2t_transformer", "dualinputmultidecoders2ttransformer_base"
)
def dualinputmultidecoders2ttransformer_base(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.input_feat_per_channel = getattr(args, "input_feat_per_channel", 80)
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
    args.conv_channels = getattr(args, "conv_channels", 1024)
    args.conv_strides = getattr(args, "conv_strides", 2)
    # Transformer
...
@register_model_architecture("dual_input_multi_decoder_s2t_transformer", "dualinputmultidecoders2ttransformer_s")
def dualinputmultidecoders2ttransformer_s(args):
...
@register_model_architecture("dual_input_multi_decoder_s2t_transformer", "dualinputmultidecoders2ttransformer_m")
def dualinputmultidecoders2ttransformer_m(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("dual_input_multi_decoder_s2t_transformer", "dualinputmultidecoders2ttransformer_b")
def dualinputmultidecoders2ttransformer_b(args):
...
@register_model_architecture("dual_input_multi_decoder_s2t_transformer", "dualinputmultidecoders2ttransformer_l")
def dualinputmultidecoders2ttransformer_l(args):
