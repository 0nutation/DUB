...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        if "src_tokens" in net_input and net_input["src_tokens"] is not None :
            src_tokens = net_input["src_tokens"]
            # length of the source text being the character length except EndOfSentence and pad
            src_lengths = (
                (src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
            )
        elif "src_txt_tokens" in net_input and net_input["src_txt_tokens"] is not None:
            src_tokens = net_input["src_txt_tokens"]
            # length of the source text being the character length except EndOfSentence and pad
            src_lengths = (
                (src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
            )
