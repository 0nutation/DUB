...
@register_criterion("guided_label_smoothed_cross_entropy_with_accuracy_textloss")
class GuidedCrossEntAccCriterion_withtextloss(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, reduce=True):
        ...
        sample_size, logging_output = self.get_logging_output(
            sample, loss, nll_loss, correct, total, speech_correct, speech_total, src_token_num, speech_loss, speech_nll_loss, attn_cost,text_loss,is_dual_input
        )
    ...
    def get_logging_output(
        ...
        if speech_loss is not None:
            ...
            logging_output["speech_correct"] = utils.item(speech_correct.data)
            logging_output["speech_total"] = utils.item(speech_total.data)
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        speech_correct_sum = sum(log.get("speech_correct", 0) for log in logging_outputs)
        speech_total_sum = sum(log.get("speech_total", 0) for log in logging_outputs)
        ...
        agg_output = {
            ...
            "text_loss": text_loss_sum / sample_size / math.log(2) if sample_size > 0 else 0.0,
            "ntokens": ntokens,
            "nsentences": nsentences,
            "nframes": nframes,
            "sample_size": sample_size,
            "acc": correct_sum * 100.0 / total_sum if total_sum > 0 else 0.0,
            "speech_acc": speech_correct_sum * 100.0 / speech_total_sum if speech_total_sum > 0 else 0.0,
            "correct": correct_sum,
            "total": total_sum,
