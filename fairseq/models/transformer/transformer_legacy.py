...
class TransformerModel(TransformerModelBase):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
            "--skip-load-embeddings",
            default=False,
            action='store_true',
            help="skip load pretrained encoder embeddings",
        )
    ...
    @classmethod
    def build_embedding(cls, args, dictionary, units_embed_dim, enc_embed_dim, post_proj, path=None):
        return super().build_embedding(
            TransformerConfig.from_namespace(args), dictionary, units_embed_dim, enc_embed_dim, post_proj, path
        )
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        ...
        # print(getattr(args, "skip_load_embeddings", False) )
        if getattr(args, "skip_load_embeddings", False):
            strict=False
        else:
            strict=True
        ...
        if pretraining_path is not None:
            if not Path(pretraining_path).exists():
                ...
                encoder = checkpoint_utils.load_pretrained_component_from_model(
                    component=encoder, checkpoint=pretraining_path, strict=strict, skip_embedding=getattr(args, "skip_load_embeddings", False)
                )
        ...
        if pretraining_path is not None and pretraining_path != "":
            if not Path(pretraining_path).exists():
                ...
                decoder = checkpoint_utils.load_pretrained_component_from_model(
                    component=decoder, checkpoint=pretraining_path,strict=strict, skip_embedding=getattr(args, "skip_load_embeddings", False)
                )
...
def transformer_wmt_en_de_big(args):
    ...
    args.units_embed_dim = getattr(args, "units_embed_dim", 1024)
    transformer_vaswani_wmt_en_de_big(args)

@register_model_architecture("transformer", "transformer_wmt_en_de_big_st")
def transformer_wmt_en_de_big_st(args):
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
    args.units_embed_dim = getattr(args, "units_embed_dim", 256)
...
@register_model_architecture("transformer", "transformer_wmt_en_de_kmemb")
def transformer_wmt_en_de_kmemb(args):
    args.units_embed_dim = getattr(args, "units_embed_dim", 768)
...
@register_model_architecture("transformer", "transformer_wmt_en_de_big_postproj")
def transformer_wmt_en_de_big_postproj(args):
    args.units_embed_dim = getattr(args, "units_embed_dim", 768)
    args.post_proj = getattr(args, "post_proj", True)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 1024)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4096)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 16)
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", False)
...
@register_model_architecture("transformer", "transformer_wmt_en_de_kmemb_big")
def transformer_wmt_en_de_kmemb_big(args):
    # args.units_embed_dim = getattr(args, "units_embed_dim", 768)
...
@register_model_architecture("transformer", "transformer_wmt_en_de_kmemb_biger")
def transformer_wmt_en_de_kmemb_biger(args):
    # args.units_embed_dim = getattr(args, "units_embed_dim", 768)
...
@register_model_architecture("transformer", "transformer_unitbart")
